...
function migrationPayload() {
  return { sourceConnectionId: sourceConnectionId.value, databases: selectedDatabases.value, maxTableSizeBytes: Math.floor(maxTableSizeMB.value * 1024 * 1024), strategy: strategy.value, createMissingTables: createMissingTables.value }
}
async function previewMigration() {
  ...
  try { migrationPlan.value = await api<DatabaseMigrationPlan>(`/connections/${props.connection.id}/migrate/preview`, { method: 'POST', body: migrationPayload() }) }
  catch (cause: unknown) { notifyError(errorMessage(cause)) }
...
async function migrate() {
  ...
  migrationResult.value = undefined
  try {
    migrationJob.value = await api<DatabaseMigrationJob>(`/connections/${props.connection.id}/migrate/jobs`, { method: 'POST', body: migrationPayload() })
...
async function pollMigration() {
  ...
  try {
    ...
    if (job.status === 'complete' && job.result) {
      ...
      if (job.result.failedTables.length) notifyInfo(t('connectionTools.migrationPartial', { migrated: job.result.tablesMigrated, failed: job.result.failedTables.length }))
  ...
  } catch { scheduleMigrationPoll(1500) }
}
...
watch([selectedDatabases, maxTableSizeMB, strategy, createMissingTables], () => { migrationPlan.value = undefined; migrationResult.value = undefined }, { deep: true })
...
<template>
  <div class="mt-5 grid max-w-4xl gap-5">
    ...
    <section class="rounded-lg border border-line bg-panel p-4">
      ...
      <div v-if="migrationPlan" class="mt-4 rounded-md border border-line bg-canvas p-3 text-sm"><h4 class="font-semibold">{{ t('connectionTools.reportTitle') }}</h4><p class="mt-1 text-muted">{{ t('connectionTools.reportSummary', { tables: migrationPlan.tables.length, rows: migrationPlan.estimatedRows, size: formatBytes(migrationPlan.totalSizeBytes) }) }}</p><details class="mt-3 overflow-hidden rounded border border-line"><summary class="cursor-pointer px-3 py-2 font-medium">{{ t('connectionTools.tablesToMigrate', { count: migrationPlan.tables.length }) }}</summary><div class="max-h-64 overflow-auto border-t border-line"><table class="min-w-full text-left text-xs"><thead class="sticky top-0 bg-panel text-muted"><tr><th class="px-3 py-2 font-medium">{{ t('table.table') }}</th><th class="px-3 py-2 text-right font-medium">{{ t('connectionTools.estimatedRows') }}</th><th class="px-3 py-2 text-right font-medium">{{ t('connectionTools.dataSize') }}</th></tr></thead><tbody><tr v-for="table in migrationPlan.tables" :key="`${table.database}.${table.table}`" class="border-t border-line"><td class="px-3 py-2 font-mono">{{ table.database }}.{{ table.table }}</td><td class="px-3 py-2 text-right">{{ table.estimatedRows }}</td><td class="px-3 py-2 text-right">{{ formatBytes(table.sizeBytes) }}</td></tr></tbody></table></div></details><details v-if="migrationPlan.skippedTables.length" class="mt-2"><summary class="cursor-pointer text-amber-700 dark:text-amber-300">{{ t('connectionTools.skippedTables', { count: migrationPlan.skippedTables.length }) }}</summary><ul class="mt-2 grid gap-1 text-xs text-muted"><li v-for="table in migrationPlan.skippedTables" :key="`${table.database}.${table.table}`">{{ table.database }}.{{ table.table }} · {{ formatBytes(table.sizeBytes) }}</li></ul></details><button type="button" class="mt-4 rounded-md bg-accent px-3 py-2 text-sm font-medium text-white disabled:opacity-50" :disabled="!migrationPlan.tables.length || migrating" @click="showMigrationConfirmation = true">{{ migrating ? t('connectionTools.migrating') : t('connectionTools.migrate') }}</button></div>
      <div v-if="migrationResult" class="mt-4 rounded-md border border-line bg-canvas p-3 text-sm"><p>{{ t('connectionTools.migrationSummary', { databases: migrationResult.databasesMigrated, tables: migrationResult.tablesMigrated, rows: migrationResult.rowsMigrated }) }}</p><details v-if="migrationResult.failedTables.length" class="mt-2"><summary class="cursor-pointer font-medium text-rose-600">{{ t('connectionTools.failedTables', { count: migrationResult.failedTables.length }) }}</summary><ul class="mt-2 grid gap-2 text-xs"><li v-for="table in migrationResult.failedTables" :key="`${table.database}.${table.table}`" class="rounded border border-rose-500/20 bg-rose-500/5 p-2"><span class="font-mono font-medium">{{ table.database }}.{{ table.table }}</span><span class="mt-1 block break-words text-muted">{{ table.error }}</span></li></ul></details><details v-if="migrationResult.skippedTables.length" class="mt-2"><summary class="cursor-pointer text-amber-700 dark:text-amber-300">{{ t('connectionTools.skippedTables', { count: migrationResult.skippedTables.length }) }}</summary><ul class="mt-2 grid gap-1 text-xs text-muted"><li v-for="table in migrationResult.skippedTables" :key="`${table.database}.${table.table}`">{{ table.database }}.{{ table.table }} · {{ formatBytes(table.sizeBytes) }}</li></ul></details></div>
    </section>
    <AppConfirmDialog v-model="showMigrationConfirmation" :title="t('connectionTools.confirmTitle')" :description="t('connectionTools.confirmDescription', { source: selectedSource?.name || '', target: connection.name, count: selectedDatabases.length, strategy: t(`connectionTools.${strategy === 'drop_recreate' ? 'dropRecreate' : 'truncateInsert'}`) })" :confirm-label="t('connectionTools.confirm')" :cancel-label="t('common.close')" tone="danger" @confirm="migrate" />
  </div>
</template>
