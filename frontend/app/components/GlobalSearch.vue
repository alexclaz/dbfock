...
const commands = computed<Command[]>(() => [
  { id: 'new-query', label: t('search.newQuery'), description: t('search.newQueryDescription'), group: t('search.actions'), icon: '＋', keywords: 'new sql query consulta', run: () => emit('newQuery') },
  ...props.tabs.map((tab) => ({
    id: `tab:${tab.id}`,
    label: tab.title,
    ...
    icon: tab.type === 'sql' ? '⌘' : tab.type === 'table' ? '▦' : tab.type === 'settings' ? '⚙' : '✦',
  ...
  { id: 'settings', label: t('search.settings'), description: t('search.openSettings'), group: t('search.settings'), icon: '⚙', keywords: 'settings preferencias', run: () => emit('openSettings') },
  { id: 'settings-appearance', label: t('settings.appearance'), description: t('search.chooseTheme'), group: t('search.settings'), icon: '◐', keywords: 'tema theme dark light auto idioma language', run: () => emit('openSettings', 'appearance') },
      ...
      <div class="scrollbar max-h-[min(60vh,32rem)] overflow-y-auto p-2">
        <template v-for="entry in groupedCommands" :key="entry.group">
          ...
          <button v-for="command in entry.commands" :key="command.id" class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left" :class="filteredCommands[selectedIndex]?.id === command.id ? 'bg-accent text-white' : 'hover:bg-canvas'" @mouseenter="selectedIndex = filteredCommands.findIndex((item) => item.id === command.id)" @click="choose(command)">
            <span class="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-canvas/70 text-sm" :class="filteredCommands[selectedIndex]?.id === command.id ? 'bg-white/15 text-white' : 'text-muted'">{{ command.icon }}</span>
