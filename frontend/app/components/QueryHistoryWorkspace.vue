...
const copiedErrorId = ref<string>()
...
async function copyError(id: string, message: string) {
  if (!navigator.clipboard) return
  try { await navigator.clipboard.writeText(message); copiedErrorId.value = id; window.setTimeout(() => { if (copiedErrorId.value === id) copiedErrorId.value = undefined }, 1500) }
