...
<template>
  <section class="scrollbar h-full overflow-auto p-5 lg:p-6">
    <div class="w-full">
      <header class="mb-6"><div class="flex items-center gap-3"><span class="grid h-10 w-10 place-items-center rounded-xl bg-accent/10 text-accent"><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 4.75A1.75 1.75 0 0 1 7.75 3h8.5A1.75 1.75 0 0 1 18 4.75V21l-6-3.5L6 21V4.75Z" /></svg></span><div><h1 class="text-xl font-semibold">{{ t('savedQueries.title') }}</h1><p class="mt-1 text-sm text-muted">{{ t('savedQueries.description') }}</p></div></div></header>
      <div v-if="queries.length" class="grid gap-3">
        <article v-for="query in queries" :key="query.id" tabindex="0" class="group cursor-pointer rounded-xl border border-line bg-panel p-4 text-left transition-colors hover:border-accent/40 hover:bg-canvas focus:outline-none focus:ring-2 focus:ring-accent/50" @click="emit('open', query.id)" @keydown.enter="emit('open', query.id)" @keydown.space.prevent="emit('open', query.id)">
          <div class="flex items-start gap-3"><span class="mt-0.5 text-accent"><svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 4.75A1.75 1.75 0 0 1 7.75 3h8.5A1.75 1.75 0 0 1 18 4.75V21l-6-3.5L6 21V4.75Z" /></svg></span><span class="min-w-0 flex-1"><span class="flex items-center justify-between gap-3"><strong class="truncate text-sm">{{ query.name }}</strong><span class="shrink-0 rounded bg-accent/10 px-2 py-0.5 text-xs text-accent">{{ t('savedQueries.connection', { name: locationFor(query) }) }}</span></span><code class="mt-1.5 block font-mono text-[11px] leading-5 text-muted whitespace-pre-wrap break-words" :class="isExpanded(query.id) ? 'max-h-80 overflow-auto pr-2' : 'max-h-10 overflow-hidden'">{{ sqlPreview(query.sql) }}</code><button type="button" class="mt-2 text-xs font-medium text-accent hover:underline" :aria-expanded="isExpanded(query.id)" @click.stop="toggleExpanded(query.id)">{{ isExpanded(query.id) ? t('savedQueries.collapseQuery') : t('savedQueries.expandQuery') }}</button></span><button type="button" class="rounded p-1.5 text-rose-500 opacity-0 transition-opacity hover:bg-rose-500/10 group-hover:opacity-100 focus:opacity-100" :title="t('savedQueries.delete')" :aria-label="t('savedQueries.delete')" @click.stop="emit('remove', query.id)"><svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6h18M9 6V4h6v2m-8 0 1 15h8l1-15M10 10v7m4-7v7" /></svg></button><span class="text-muted opacity-0 transition-opacity group-hover:opacity-100">→</span></div>
        </article>
      </div>
      <div v-else class="rounded-xl border border-dashed border-line px-6 py-16 text-center"><div class="text-muted"><svg class="mx-auto h-7 w-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 4.75A1.75 1.75 0 0 1 7.75 3h8.5A1.75 1.75 0 0 1 18 4.75V21l-6-3.5L6 21V4.75Z" /></svg></div><h2 class="mt-3 font-medium">{{ t('savedQueries.emptyTitle') }}</h2><p class="mt-1 text-sm text-muted">{{ t('savedQueries.emptyDescription') }}</p></div>
    </div>
  </section>
</template>
