...
import { editorFontOptions, interfaceFontOptions, type EditorFont, type InterfaceFont } from '~/composables/useFontPreferences'
...
const { update, currentVersion, checking: checkingForUpdate, checkStatus: updateCheckStatus, isWails, checkForUpdate, openWailsUpdate } = useAppUpdate()
...
const { interfaceFont, editorFont, setInterfaceFont, setEditorFont } = useFontPreferences()
const shortcuts = [
...
async function checkForAppUpdate() {
  ...
  if (available) { notifySuccess(t('update.availableNotice', { version: available.latestVersion })); return }
  if (updateCheckStatus.value === 'up-to-date') { notifySuccess(t('update.upToDate', { version: currentVersion.value || appVersion })); return }
  notifyError(t('update.checkError'))
}
function prepareUpdate() { if (openWailsUpdate()) notifySuccess(t('update.updateOpening')) }
