...
<template>
  <div class="flex h-full min-h-0 flex-col">
    ...
    <QueryResults :result-tabs="resultTabs" :active-result-tab-id="activeResultTabId" :loading="running" :loading-more="loadingMoreRows" :can-create-tab="true" :summary="resultSummary" @select-tab="emit('selectResultTab', $event)" @close-tab="emit('closeResultTab', $event)" @create-tab="emit('createResultTab')" @copy="emit('copyResult', $event)" @save="(id, result) => emit('saveResult', id, result)" @load-more="emit('loadMore')" />
  </div>
</template>
