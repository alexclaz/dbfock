...
import type { QueryResult, TableStructure } from '~/types/database'
import { queryResultAsCSV, queryResultAsJSON, queryResultAsTSV, queryResultEdits } from '~/utils/queryResult'

type TableSection = 'data' | 'structure' | 'constraints' | 'foreignKeys' | 'references' | 'triggers' | 'indexes' | 'ddl'

const props = defineProps<{ connectionId: string; database: string; table: string; activeSection?: TableSection }>()
const emit = defineEmits<{ 'update:activeSection': [value: TableSection]; transactionStatus: [connectionId: string, pending: boolean, pendingStatements: number] }>()
const api = useApi()
const { t } = useI18n()
...
const tableSections: TableSection[] = ['data', 'structure', 'constraints', 'foreignKeys', 'references', 'triggers', 'indexes', 'ddl']
...
function selectSection(next: TableSection) {
  ...
  if (next !== 'data' && !structure.value) void loadStructure()
}
...
<template>
  <section class="flex h-full min-h-0 flex-col">
    ...
    <pre v-else class="scrollbar flex-1 overflow-auto whitespace-pre-wrap p-4 font-mono text-xs">{{ structure?.ddl || t('table.loadingDdl') }}</pre>
  </section>
</template>
