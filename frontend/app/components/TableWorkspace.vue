...
<template>
  <section class="flex h-full min-h-0 flex-col">
    ...
    <template v-if="section === 'data'"><div class="flex items-center justify-between border-b border-line px-4 py-2 text-xs text-muted"><span>{{ t('query.rows', { count: result?.rowCount || 0 }) }}{{ result?.hasMore ? '+' : '' }}</span><div class="flex items-center gap-2"><AppSelect v-model="pageSize" :disabled="loading" class="w-24" :options="[{ value: 50, label: '50' }, { value: 100, label: '100' }, { value: 250, label: '250' }]" @change="page = 0; loadData()" /><button :disabled="page === 0 || loading" @click="previousPage">{{ t('table.previous') }}</button><button :disabled="!result?.hasMore || loading" @click="nextPage">{{ t('table.next') }}</button><button :disabled="loading" @click="loadData">↻</button></div></div><div class="min-h-0 flex-1"><DataGrid :result="result" :loading="loading" /></div></template>
