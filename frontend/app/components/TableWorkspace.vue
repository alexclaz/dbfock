...
const structureCopied = ref<string>()
const ddlEl = ref<HTMLElement>()
const importInput = ref<HTMLInputElement>()
const selectedImportFile = ref<File>()
const transferring = ref(false)
const importOptions = reactive({ truncateBefore: false, ignoreDuplicates: false })
const pendingImport = ref<{ data: ImportedTableRows; columnTypes: Record<string, string> }>()
const showImportTruncateConfirmation = ref(false)
...
async function copyTableName() {
  ...
  try { await navigator.clipboard.writeText(props.table); nameCopied.value = true; window.setTimeout(() => nameCopied.value = false, 1500) }
  catch { /* Clipboard access can be denied by the browser. */ }
}
async function copyStructureValue(id: string, value: string) {
  if (!navigator.clipboard) return
  try {
    await navigator.clipboard.writeText(value)
    structureCopied.value = id
    window.setTimeout(() => { if (structureCopied.value === id) structureCopied.value = undefined }, 1500)
  } catch { /* Clipboard access can be denied by the browser. */ }
}
...
async function writeImport(imported: ImportedTableRows, columnTypes: Record<string, string>, truncateBefore: boolean) {
  transferring.value = true
  try {
    if (truncateBefore) await api<QueryResult>(`/connections/${props.connectionId}/query`, { method: 'POST', body: { sql: `TRUNCATE TABLE ${tableNameSQL()}`, historySql: `truncate ${props.database}.${props.table} before import` } })
    let importedRows = 0
    let transaction: QueryResult | undefined
...
async function importTable() {
  ...
  if (!file) { notifyError(t('table.tools.importFileRequired')); return }
  if (transferring.value) return
  ...
  try {
    ...
    if (importOptions.truncateBefore) { pendingImport.value = { data: imported, columnTypes }; showImportTruncateConfirmation.value = true; return }
    await writeImport(imported, columnTypes, false)
  } catch (cause: any) { notifyError(cause.message) }
  finally { transferring.value = false }
}
async function confirmImportTruncate() {
  showImportTruncateConfirmation.value = false
  const pending = pendingImport.value
  pendingImport.value = undefined
  if (pending) await writeImport(pending.data, pending.columnTypes, true)
}
...
<template>
  <section class="flex h-full min-h-0 flex-col">
    ...
    <div v-else-if="section === 'structure'" class="scrollbar overflow-auto p-4">
      ...
      <table class="min-w-full text-left text-sm"><thead class="text-xs text-muted"><tr><th class="border-b border-line p-2">{{ t('table.column') }}</th><th class="border-b border-line p-2">{{ t('table.type') }}</th><th class="border-b border-line p-2">{{ t('table.nullable') }}</th><th class="border-b border-line p-2">{{ t('table.key') }}</th><th class="border-b border-line p-2">{{ t('table.default') }}</th><th class="border-b border-line p-2"><span class="sr-only">{{ t('grid.actions') }}</span></th></tr></thead><tbody><tr v-for="column in structure?.columns" :key="column.name"><td class="group border-b border-line p-2 font-medium"><div class="flex items-center gap-1"><span>{{ column.name }}</span><button type="button" class="grid rounded p-1 text-muted opacity-0 transition hover:bg-canvas hover:text-ink focus-visible:opacity-100 group-hover:opacity-100" :title="structureCopied === `${column.name}:name` ? t('grid.copied') : t('grid.copyColumn')" :aria-label="structureCopied === `${column.name}:name` ? t('grid.copied') : t('grid.copyColumn')" @click="copyStructureValue(`${column.name}:name`, column.name)"><Icon :name="structureCopied === `${column.name}:name` ? 'lucide:check' : 'lucide:copy'" class="h-3.5 w-3.5" aria-hidden="true" /></button></div></td><td class="group border-b border-line p-2 text-muted"><div class="flex items-center gap-1"><span>{{ column.columnType }}</span><button type="button" class="grid rounded p-1 opacity-0 transition hover:bg-canvas hover:text-ink focus-visible:opacity-100 group-hover:opacity-100" :title="structureCopied === `${column.name}:type` ? t('grid.copied') : t('grid.copy')" :aria-label="structureCopied === `${column.name}:type` ? t('grid.copied') : t('grid.copy')" @click="copyStructureValue(`${column.name}:type`, column.columnType)"><Icon :name="structureCopied === `${column.name}:type` ? 'lucide:check' : 'lucide:copy'" class="h-3.5 w-3.5" aria-hidden="true" /></button></div></td><td class="group border-b border-line p-2"><div class="flex items-center gap-1"><span>{{ column.nullable ? t('table.yes') : t('table.no') }}</span><button type="button" class="grid rounded p-1 opacity-0 transition hover:bg-canvas hover:text-ink focus-visible:opacity-100 group-hover:opacity-100" :title="structureCopied === `${column.name}:nullable` ? t('grid.copied') : t('grid.copy')" :aria-label="structureCopied === `${column.name}:nullable` ? t('grid.copied') : t('grid.copy')" @click="copyStructureValue(`${column.name}:nullable`, column.nullable ? t('table.yes') : t('table.no'))"><Icon :name="structureCopied === `${column.name}:nullable` ? 'lucide:check' : 'lucide:copy'" class="h-3.5 w-3.5" aria-hidden="true" /></button></div></td><td class="group border-b border-line p-2"><div class="flex items-center gap-1"><span>{{ column.key || '—' }}</span><button v-if="column.key" type="button" class="grid rounded p-1 opacity-0 transition hover:bg-canvas hover:text-ink focus-visible:opacity-100 group-hover:opacity-100" :title="structureCopied === `${column.name}:key` ? t('grid.copied') : t('grid.copy')" :aria-label="structureCopied === `${column.name}:key` ? t('grid.copied') : t('grid.copy')" @click="copyStructureValue(`${column.name}:key`, column.key)"><Icon :name="structureCopied === `${column.name}:key` ? 'lucide:check' : 'lucide:copy'" class="h-3.5 w-3.5" aria-hidden="true" /></button></div></td><td class="group border-b border-line p-2"><div class="flex items-center gap-1"><span>{{ column.default ?? '—' }}</span><button v-if="column.default !== null" type="button" class="grid rounded p-1 opacity-0 transition hover:bg-canvas hover:text-ink focus-visible:opacity-100 group-hover:opacity-100" :title="structureCopied === `${column.name}:default` ? t('grid.copied') : t('grid.copy')" :aria-label="structureCopied === `${column.name}:default` ? t('grid.copied') : t('grid.copy')" @click="copyStructureValue(`${column.name}:default`, column.default)"><Icon :name="structureCopied === `${column.name}:default` ? 'lucide:check' : 'lucide:copy'" class="h-3.5 w-3.5" aria-hidden="true" /></button></div></td><td class="border-b border-line p-2"><div class="flex justify-end gap-1"><button type="button" class="grid rounded p-1.5 text-muted hover:bg-canvas hover:text-ink disabled:opacity-50" :title="t('table.structureEdit')" :aria-label="t('table.structureEdit')" :disabled="alteringColumn" @click="openEditColumn(column)"><Icon name="lucide:pencil" class="h-4 w-4" aria-hidden="true" /></button><button type="button" class="grid rounded p-1.5 text-rose-600 hover:bg-rose-500/10 disabled:opacity-50" :title="t('table.structureDrop')" :aria-label="t('table.structureDrop')" :disabled="alteringColumn" @click="requestDropColumn(column)"><Icon name="lucide:trash-2" class="h-4 w-4" aria-hidden="true" /></button></div></td></tr></tbody></table>
    </div>
