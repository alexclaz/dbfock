    ...
    <div v-if="props.modelValue" class="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4" @mousedown.self="emit('update:modelValue', false)">
      <section class="flex max-h-[calc(100vh-2rem)] w-full max-w-3xl flex-col rounded-xl border border-line bg-panel p-5 shadow-panel" role="dialog" aria-modal="true" :aria-label="t('transaction.confirmTitle')">
        ...
        <div class="mt-4 min-h-0 overflow-auto rounded-lg border border-line">
          <div class="sticky top-0 flex items-center gap-3 border-b border-line bg-panel px-3 py-2 text-xs font-medium text-muted">
          ...
          <div v-for="(statement, index) in statements" :key="statement.id" class="flex gap-3 border-b border-line p-3 last:border-b-0">
            <input v-model="selected" type="checkbox" :value="statement.id" :disabled="committing" :aria-label="t('transaction.selectStatement', { count: index + 1 })">
            ...
            <div class="flex shrink-0 items-start gap-2">
              <button class="rounded px-2 py-1 text-xs text-rose-700 hover:bg-rose-500/10 dark:text-rose-300" :disabled="committing" @click="rollback([statement.id])">{{ t('transaction.rollback') }}</button>
              <button class="rounded bg-amber-600 px-2 py-1 text-xs font-medium text-white disabled:opacity-60" :disabled="committing" @click="commit([statement.id])">{{ t('transaction.confirmOne') }}</button>
            </div>
          </div>
        </div>
