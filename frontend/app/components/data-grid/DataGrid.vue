...
<template>
  <div class="scrollbar h-full overflow-auto" @scroll="loadMore">
    ...
    <table v-else-if="view === 'table'" class="table-fixed border-collapse text-left text-sm">
      ...
      <thead class="sticky top-0 bg-panel text-xs text-muted"><tr><th class="w-12 border-b border-r border-line px-3 py-2 font-medium">#</th><th v-for="column in columns" :key="column.name" class="relative border-b border-r border-line px-3 py-2 font-medium" :style="{ width: `${columnWidth(column)}px`, maxWidth: `${columnWidth(column)}px` }"><div class="flex items-center gap-1"><span class="min-w-0 flex-1 truncate">{{ column.name }}</span><button type="button" class="grid h-5 w-5 shrink-0 place-items-center rounded hover:bg-canvas hover:text-ink" :title="copiedColumn === column.name ? t('grid.copied') : t('grid.copyColumn')" :aria-label="copiedColumn === column.name ? t('grid.copied') : t('grid.copyColumn')" @click="copyColumnName(column.name)"><Icon :name="copiedColumn === column.name ? 'lucide:check' : 'lucide:copy'" class="h-3 w-3" aria-hidden="true" /></button><button v-if="canSort" type="button" class="grid h-5 w-5 shrink-0 place-items-center rounded hover:bg-canvas hover:text-ink" :class="sortColumn === column.name ? 'text-accent' : ''" :title="t('grid.sortColumn')" :aria-label="t('grid.sortColumn')" @click="$emit('sort', column.name)"><Icon :name="sortColumn === column.name ? (sortDirection === 'desc' ? 'lucide:arrow-down' : 'lucide:arrow-up') : 'lucide:arrow-up-down'" class="h-3 w-3" aria-hidden="true" /></button></div><small class="block truncate font-normal opacity-70">{{ column.databaseType }}</small><span class="absolute inset-y-0 -right-1 z-10 w-2 cursor-col-resize select-none hover:bg-accent/70 active:bg-accent" :title="t('grid.resizeColumn')" @pointerdown="resizeColumn($event, column)" /></th></tr></thead>
      <tbody class="select-text"><tr v-for="(row,index) in rows" :key="index" class="hover:bg-accent/5"><td class="border-b border-r border-line px-3 py-2 text-xs text-muted">{{ index + 1 }}</td><td v-for="column in columns" :key="column.name" class="overflow-hidden border-b border-r border-line px-3 py-2" :class="row[column.name] === null ? 'italic text-muted' : ''" :style="{ width: `${columnWidth(column)}px`, maxWidth: `${columnWidth(column)}px` }" :title="display(row[column.name])" @dblclick="editCell(index, column.name)"><input v-if="editing && activeCell?.row === index && activeCell.column === column.name" :ref="(element) => { if (element) inputRefs.set(cellKey(index, column.name), element as HTMLInputElement) }" class="-my-1 w-full rounded border border-accent bg-canvas px-1 py-1 text-sm text-ink outline-none" :value="inputValue(row[column.name])" @input="updateCell(index, column.name, ($event.target as HTMLInputElement).value)" @blur="finishCell" @keydown.enter.prevent="finishCell" @keydown.esc.prevent="resetCell"><span v-else class="block truncate">{{ display(row[column.name]) }}</span></td></tr><tr v-if="loadingMore"><td :colspan="columns.length + 1" class="p-3 text-center text-xs text-muted">{{ t('grid.loading') }}</td></tr><tr v-if="!rows.length"><td :colspan="columns.length + 1" class="p-8 text-center text-muted">{{ t('grid.noRows') }}</td></tr></tbody>
    </table>
