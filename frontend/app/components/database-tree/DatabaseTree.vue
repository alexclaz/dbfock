...
<template>
  <aside class="flex h-full shrink-0 flex-col border-r border-line bg-panel" :style="{ width: `${width ?? 288}px` }">
    <div class="flex shrink-0 items-center justify-between border-b border-line px-3 py-3"><div class="flex items-center gap-2"><img class="h-8 w-8 rounded-lg border border-line bg-white object-contain p-0.5" src="/branding/favicon/android-chrome-192x192.png" alt="DBFock" /><span class="font-semibold tracking-tight">DBFock</span></div><div class="flex items-center gap-1"><button class="focus-ring grid h-9 w-9 place-items-center rounded-md text-lg text-muted hover:bg-canvas hover:text-ink" :title="t('tree.home')" :aria-label="t('tree.home')" @click="$emit('home')">⌂</button><button class="focus-ring grid h-9 w-9 place-items-center rounded-md text-lg text-muted hover:bg-canvas hover:text-ink" :title="t('tree.savedQueries')" :aria-label="t('tree.savedQueries')" @click="$emit('saved')"><svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 4.75A1.75 1.75 0 0 1 7.75 3h8.5A1.75 1.75 0 0 1 18 4.75V21l-6-3.5L6 21V4.75Z" /></svg></button><button class="focus-ring grid h-9 w-9 place-items-center rounded-md text-lg text-muted hover:bg-canvas hover:text-ink" :title="t('tree.smartQueries')" :aria-label="t('tree.smartQueries')" @click="$emit('smart')">✦</button><button class="focus-ring grid h-9 w-9 place-items-center rounded-md text-lg text-muted hover:bg-canvas hover:text-ink" :title="t('tree.settings')" :aria-label="t('tree.settings')" @click="$emit('settings')">⚙</button></div></div>
    <div class="flex items-center gap-2 border-b border-line p-3"><input v-model="search" class="focus-ring h-8 min-w-0 flex-1 rounded-md border border-line bg-canvas px-2 text-sm" :placeholder="t('tree.searchConnections')"><button class="rounded p-1 text-muted hover:bg-canvas" :title="t('tree.refreshConnections')" @click="$emit('refresh')">↻</button></div>
    <div class="scrollbar flex-1 overflow-auto px-2 py-3">
      <div class="mb-2 flex items-center justify-between px-2"><p class="text-[11px] font-semibold uppercase tracking-wider text-muted">{{ t('tree.connections') }}</p><button class="rounded px-1.5 py-0.5 text-base leading-none text-muted hover:bg-canvas hover:text-ink" :title="t('tree.newConnection')" @click="$emit('add')">+</button></div>
      <div v-for="connection in filteredConnections" :key="connection.id">
        <div class="group relative flex items-center gap-1 rounded-md px-1 py-1 hover:bg-canvas" :class="activeConnectionId === connection.id ? 'bg-canvas' : ''">
          <button class="w-5 text-xs text-muted disabled:opacity-40" :disabled="connection.status !== 'connected'" @click="toggleConnection(connection)">{{ expanded.has(`c:${connection.id}`) ? '⌄' : '›' }}</button>
          ...
          <button class="rounded px-1 text-xs text-muted hover:bg-line disabled:opacity-60" :aria-expanded="actionMenuFor === connection.id" :aria-label="t('tree.connectionActions')" :disabled="changingConnectionId === connection.id" @click.stop="actionMenuFor = actionMenuFor === connection.id ? undefined : connection.id">•••</button>
        ...
        <div v-if="expanded.has(`c:${connection.id}`)" class="ml-3 border-l border-line pl-2"><p v-if="loading.has(`c:${connection.id}`)" class="px-2 py-1 text-xs text-muted">{{ t('tree.loadingDatabases') }}</p><template v-for="database in databases[connection.id]" :key="database.name"><div class="flex cursor-pointer items-center gap-1 rounded px-1 py-1 hover:bg-canvas" @click="toggleDatabase(connection,database.name)"><button class="w-5 text-xs text-muted" @click.stop="toggleDatabase(connection,database.name)">{{ expanded.has(`d:${connection.id}:${database.name}`) ? '⌄' : '›' }}</button><button class="truncate text-left text-sm">◇ {{ database.name }}</button></div><div v-if="expanded.has(`d:${connection.id}:${database.name}`)" class="ml-3 border-l border-line pl-2"><p v-if="loading.has(`d:${connection.id}:${database.name}`)" class="px-2 py-1 text-xs text-muted">{{ t('tree.loadingTables') }}</p><button v-for="table in tables[`d:${connection.id}:${database.name}`]" :key="table.name" class="flex w-full items-center gap-2 rounded px-2 py-1 text-left text-sm text-muted hover:bg-canvas hover:text-ink" @click="$emit('table', connection, database.name, table.name)"><span>▦</span>{{ table.name }}</button></div></template></div>
      </div>
      <div v-if="!filteredConnections.length" class="whitespace-pre-line px-2 py-8 text-center text-sm text-muted">{{ t('tree.empty') }}</div>
    </div>
  </aside>
  <div v-if="toast" role="alert" class="fixed bottom-4 right-4 z-50 flex max-w-sm items-start gap-3 rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700 shadow-panel dark:border-rose-900 dark:bg-rose-950 dark:text-rose-200"><span class="min-w-0 flex-1">{{ toast }}</span><button class="-mr-1 -mt-1 rounded p-1 leading-none hover:bg-rose-100 dark:hover:bg-rose-900" :aria-label="t('common.close')" @click="toast = ''">×</button></div>
</template>
