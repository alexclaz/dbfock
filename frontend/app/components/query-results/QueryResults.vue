...
function canModifyRowsFor(resultTab: ResultTab) { return Boolean(resultTab.sources?.length === 1 && resultTab.view === 'table' && !searchActiveFor(resultTab)) }
...
<template>
  ...
  <div class="min-h-0 flex-1"><DataGrid v-for="resultTab in resultTabs" v-show="resultTab.id === activeResultTab?.id" :key="resultTab.id" :ref="(grid) => setResultGrid(resultTab.id, grid as unknown as ResultGrid | null)" :result="displayedResultFor(resultTab)" :loading="loading" :loading-more="loadingMore" :view="resultTab.view" :editing="resultTab.editing" :editable="Boolean(resultTab.sources?.length) && !searchActiveFor(resultTab)" :editable-columns="resultTab.sources?.flatMap((source) => source.columns)" :primary-key="resultTab.sources?.[0]?.primaryKey" :json-editable="!resultTab.sources?.length" :sortable="!resultTab.editing" :sort-column="resultTab.sortColumn" :sort-direction="resultTab.sortDirection" :row-actions="canModifyRowsFor(resultTab)" @load-more="emit('loadMore')" @start-edit="resultTab.editing = true" @dirty="resultTab.dirty = $event" @save="(result, mutations) => emit('save', resultTab.id, result, mutations)" @cancel="resultTab.editing = false" @sort="(column, direction) => sortResult(resultTab, column, direction)" /></div>
