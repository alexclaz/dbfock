...
const props = withDefaults(defineProps<{ modelValue: string; connectionId: string; connectionName: string; connections?: Connection[]; executionConnectionId?: string; initialDatabase?: string; running?: boolean; split?: boolean; width?: number | string; queryActions?: boolean; production?: boolean; transactionPending?: boolean; pendingStatements?: number; committing?: boolean }>(), { connections: () => [] })
const emit = defineEmits<{ 'update:modelValue': [value: string]; 'update:executionConnectionId': [value?: string]; execute: [sql: string, newResultTab?: boolean]; explain: [sql: string]; improve: [sql: string]; createSmartQuery: [sql: string]; sendToChat: [sql: string]; newQuery: []; saveQuery: []; commit: []; rollback: [] }>()
const api = useApi()
const { t } = useI18n()
