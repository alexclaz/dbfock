...
<template>
  <div class="flex h-10 overflow-hidden border-b border-line bg-panel" @click="closeContextMenu">
    <div v-for="tab in tabs" :key="tab.id" :draggable="!isPinned(tab)" class="group relative -mb-px flex h-10 min-w-0 flex-1 items-center overflow-hidden border-b-2 border-r border-line px-3 text-sm transition-colors" :class="activeId === tab.id ? 'border-b-accent bg-accent/10 font-semibold text-ink shadow-[inset_0_1px_0_rgba(59,130,246,0.2)]' : 'border-b-transparent text-muted hover:bg-canvas/60 hover:text-ink'" @contextmenu.prevent.stop="openContextMenu($event, tab)" @dragstart="startDrag($event, tab)" @dragover.prevent="allowMoveDrop" @drop.prevent="{ const id = $event.dataTransfer?.getData('text/plain'); if (id) emit('reorder', id, tab.id) }">
      <button type="button" class="flex min-w-0 flex-1 items-center gap-1.5 text-left" :aria-current="activeId === tab.id ? 'page' : undefined" @click.stop="selectTab(tab.id)">
        <Icon v-if="!isPinned(tab)" name="lucide:grip-vertical" class="h-4 w-4 shrink-0 cursor-grab text-muted" :title="t('tabs.dragToReorder')" aria-hidden="true" />
      ...
      <button type="button" class="ml-1 shrink-0 rounded p-1 text-muted opacity-0 group-hover:opacity-100 hover:bg-line focus:opacity-100" :aria-label="t('common.close')" @click.stop="closeTab(tab.id)"><Icon name="lucide:x" class="h-3.5 w-3.5" aria-hidden="true" /></button>
    </div>
