...
function tableDeleted(connectionId: string, database: string, table: string) {
  workspace.closeTabs(new Set(workspace.tabs.filter((tab) => tab.type === 'table' && tab.connectionId === connectionId && tab.database === database && tab.table === table).map((tab) => tab.id)))
  void databaseTree.value?.refreshConnection(connectionId)
}
