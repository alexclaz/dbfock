...
export interface DatabaseUser { username: string; host: string; authPlugin: string; locked: boolean; passwordExpired: boolean; grants: string[] }
export interface DatabaseUserInput { username: string; host: string; password?: string; privileges: string[]; databases: string[]; database?: string }
...
export interface QueryTabHistory { id: string; title: string; connectionId: string; executionConnectionId?: string; sql: string; queryNumber?: number; savedQueryId?: string; closedAt: string }
...
export interface AIAgentChat { draft: string; messages: AIAgentMessage[]; includeEditorQuery?: boolean; fastSchemaRetrieval?: boolean; databaseScope?: 'all' | 'selected'; selectedDatabases?: string[]; tableScope?: 'all' | 'selected'; selectedTables?: AISchemaTable[]; scopeConfirmation?: AIScopeConfirmation }
export interface AIChatJob { id: string; status: 'running' | 'complete' | 'failed'; message?: string; partialMessage?: string; error?: string; createdAt: string; updatedAt: string }
export interface WorkspaceTab { id: string; title: string; type: 'empty' | 'welcome' | 'saved' | 'smart' | 'history' | 'sql' | 'table' | 'database' | 'connection-home' | 'settings' | 'stats'; connectionId?: string; executionConnectionId?: string; database?: string; table?: string; sql?: string; sqlScrollTop?: number; sqlScrollLeft?: number; queryNumber?: number; tableSection?: 'data' | 'structure' | 'constraints' | 'foreignKeys' | 'references' | 'triggers' | 'indexes' | 'ddl' | 'diagram' | 'tools'; databaseSection?: 'tables' | 'diagram' | 'tools'; settingsSection?: 'appearance' | 'shortcuts' | 'connections' | 'ai' | 'audit' | 'backup' | 'about'; aiChat?: AIAgentChat; aiJobId?: string; aiStatus?: 'running' | 'complete'; dirty?: boolean; savedQueryId?: string }
